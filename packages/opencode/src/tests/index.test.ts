        ...
        if (errorEvent) {
          const event = `event: error\ndata: ${JSON.stringify({
            ...
            error: {
              ...
              ...(errorEvent.status === undefined
                ? {}
                : { status: errorEvent.status }),
              ...(errorEvent.message === undefined
                ? {}
...
describe('fallback Claustrum credential resolution', () => {
  const originalFetch = globalThis.fetch
  afterEach(() => {
    globalThis.fetch = originalFetch
  })

  function persistedRefreshError(accountIdentity: string) {
    return buildRefreshOperationError({
  ...
  async function bootMainVault401(
    ...
    globalThis.fetch = mock(async (_input: unknown, init?: RequestInit) => {
      ...
      await options.onMessageRequest?.(authorizations.length, init)
      return new Response('{}', { status: responseStatuses.shift() ?? 401 })
    }) as unknown as typeof fetch
  ...
  describe('vault-served main routes generically', () => {
    ...
    test.serial(
      'uses the rotated main vault bearer for quota after a successful 401 retry',
      async () => {
        ...
        await expectHandledCommandResponse(
          fixture.plugin['command.execute.before']({
            command: 'claude-quota',
            arguments: '',
            sessionID: 'vault-main-retried-bearer',
          }),
        )
  ...
  test.serial('warms a manifest-only handle at startup', async () => {
    ...
    const plugin = await getPlugin(undefined, undefined, {
      claustrumConnector: manifestConnector(
        calls,
        new Map([[manifestHandle, 'startup-access']]),
      ),
    })
    expect(
  ...
  test.serial(
    ...
    async () => {
      ...
      expect(intervals).toHaveLength(baseline)
      await expect(
        result.fetch(MESSAGES_URL, EMPTY_POST),
      ).rejects.toMatchObject({
        code: 'custody_state_mismatch',
      ...
      const payload = await runCustodyCommand(plugin, 'boot-dark-main', '')
      expect(payload?.text).toContain('FAIL_CLOSED')
      await plugin.dispose?.()
    },
  )

  test.serial(
    'keeps LOCAL_SERVE refresh intervals and transport unchanged',
    async () => {
      await useTempAccountFile(
        createFallbackStorage({ claustrum: { mode: 'local' } }),
      )
      const intervals: unknown[] = []
      const authorizations: string[] = []
      ...
      const intervalsBeforeLoader = intervals.length
      const result = await plugin.auth.loader(
        () =>
          Promise.resolve({
            type: 'oauth' as const,
      ...
      const response = await result.fetch(MESSAGES_URL, EMPTY_POST)
      expect(response.status).toBe(200)
      expect(intervals).toHaveLength(intervalsBeforeLoader + 1)
      expect(authorizations).toContain('Bearer local-main-access')
      await plugin.dispose?.()
    },
  )

  test.serial('keeps CLAUSTRUM_SERVE on the resident vault path', async () => {
    await useTempAccountFile(
      createFallbackStorage({ claustrum: { mode: 'claustrum' }, accounts: [] }),
    )
    await writeManifest([{ label: 'main', handle: manifestHandle }])
    const intervals: unknown[] = []
    const authorizations: string[] = []
    ...
    const plugin = await getPlugin(undefined, undefined, {
      claustrumConnector: manifestConnector(
        [],
        new Map([[manifestHandle, 'vault-main-access']]),
      ),
      setInterval: mock((handler: () => unknown) => {
        intervals.push(handler)
        return { unref() {} } as never
    ...
    const response = await result.fetch(MESSAGES_URL, EMPTY_POST)
    expect(response.status).toBe(200)
    expect(intervals).toHaveLength(intervalsBeforeLoader)
    expect(authorizations).toContain('Bearer vault-main-access')
    await plugin.dispose?.()
  })
  ...
  test.serial(
    'retries a 401 with the post-rotation version when a proactive refresh returns stale',
    async () => {
      let credentialGets = 0
      let releaseProactiveRefresh!: () => void
      const proactiveRefresh = new Promise<void>((resolve) => {
        releaseProactiveRefresh = resolve
      })
      let proactiveRefreshStarted!: () => void
      const proactiveRefreshStartedPromise = new Promise<void>((resolve) => {
        proactiveRefreshStarted = resolve
      })
      let releaseFirstResponse!: () => void
      ...
      const fixture = await bootMainVault401({
        ...
        credentialGet: async () => {
          ...
          if (credentialGets === 2) {
            proactiveRefreshStarted()
            await proactiveRefresh
            return credentialResponse('vault-main-access-v17', 17)
          }
          return credentialResponse('vault-main-access-v18', 18)
        },
        onMessageRequest: async (count) => {
          if (count !== 1) return
          firstRequestStarted()
          await firstResponse
        },
      })
      const cache = fixture.plugin.__claustrumCredentialCache

      await cache.get(manifestHandle)
      await proactiveRefreshStartedPromise
      const responsePromise = fixture.result.fetch(MESSAGES_URL, EMPTY_POST)
      await firstRequestStartedPromise
      releaseFirstResponse()
      await Promise.resolve()
      releaseProactiveRefresh()
  ...
  test.serial(
    'returns the original 401 instead of replaying a streamed vault request body',
    async () => {
      let recordVersion = 17
      const fixture = await bootMainVault401({
        responseStatuses: [401, 200],
      ...
      const stream = new ReadableStream({
        start(controller) {
          controller.enqueue(new TextEncoder().encode('one-shot body'))
          controller.close()
        },
      })
  ...
  test.serial(
    'holds an unusable main tombstone dark before route-specific refusal',
    async () => {
      for (const fixture of [
        {
          name: 'reauth',
  ...
  test.serial(
    'does not poll a handle minted after a fallback-driven dark boot',
    async () => {
      await useTempAccountFile(manifestStorage({ label: 'minted' }))
      process.env.CLAUSTRUM_OPENCODE_HANDLES = join(
        tempConfigDir!,
        'handles.json',
      )
      ...
      const plugin = await getPlugin(undefined, undefined, {
        claustrumConnector: manifestConnector(
          calls,
          new Map([[manifestHandle, 'minted-access']]),
        ),
        setInterval: mock((handler: () => unknown) => {
      ...
      const path = await writeManifest([
        { label: 'minted', handle: manifestHandle },
      ])
  ...
  test.serial(
    'keeps a foreign manifest on sidecar and warns once',
    async () => {
      await useTempAccountFile(
        manifestStorage({ label: 'foreign', legacy: legacyHandle }),
      )
      await writeManifest(
        [{ label: 'foreign', handle: manifestHandle }],
        'foreign',
      )
      ...
      const handlers: Array<() => unknown> = []
      const plugin = await getPlugin(undefined, undefined, {
        claustrumConnector: manifestConnector(calls, new Map()),
        setInterval: mock((handler: () => unknown) => {
          handlers.push(handler)
          return { unref() {} } as never
        }) as never,
      })
      for (let tick = 0; tick < 3; tick++) await handlers.at(-1)?.()
      expect(
        calls.filter((call) => call.method === 'credential.get'),
      ).toHaveLength(0)
      expect(
  ...
  test.serial('uses legacy on an invalid manifest and warns once', async () => {
    await useTempAccountFile(
      manifestStorage({ label: 'invalid', legacy: legacyHandle }),
    )
    ...
    const plugin = await getPlugin(undefined, undefined, {
      claustrumConnector: manifestConnector(
        calls,
        new Map([[legacyHandle, 'legacy-access']]),
      ),
      setInterval: mock((handler: () => unknown) => {
        handlers.push(handler)
        return { unref() {} } as never
    ...
    for (let tick = 0; tick < 3; tick++) await handlers.at(-1)?.()
    expect(
      calls.filter((call) => call.method === 'credential.get').length,
    ).toBeGreaterThan(0)
    expect(
      logs.filter(
        (record) =>
  ...
  test.serial(
    ...
    async () => {
      ...
      const plugin = await getPlugin(undefined, undefined, {
        claustrumConnector: manifestConnector(
          calls,
          new Map([[manifestHandle, 'gate-access']]),
        ),
        setInterval: mock((handler: () => unknown) => {
          handlers.push(handler)
          return { unref() {} } as never
  ...
  test.serial(
    ...
    async () => {
      ...
      try {
        ...
        const localResult = await plugin.auth.loader(
          () => Promise.resolve(custodyTombstoneOAuth('anthropic')),
          { models: {} },
        )
        await expect(
          localResult.fetch('https://example.test', {}),
        ).rejects.toMatchObject({
          code: 'custody_state_mismatch',
  ...
  async function runCustodyCommand(
    ...
    return drainNotifications(0, sessionID).at(-1)?.payload
  }

  async function clearAfterConcurrentSnapshot(
    accountId: string,
    handle: string,
    concurrent: AccountStorage,
    path: string,
  ) {
    const statePath = getAccountStatePath(path)
    const staleState = await readFile(statePath, 'utf8')
    const concurrentPath = join(dirname(path), `${accountId}-concurrent.json`)
    await saveAccounts(concurrent, concurrentPath)
    const concurrentConfig = await readFile(concurrentPath, 'utf8')
    const concurrentState = await readFile(
      getAccountStatePath(concurrentPath),
      'utf8',
    )
    ...
    if (!configLock) throw new Error('failed to hold account config lock')

    await rm(statePath, { force: true })
    const fifo = Bun.spawnSync(['mkfifo', statePath])
    expect(fifo.exitCode).toBe(0)
    const clearPromise = clearClaustrumRefreshErrorPersistent(
      accountId,
      handle,
      path,
    )
    const writer = Bun.spawn([
      process.execPath,
      '--eval',
      `import { writeFile } from 'node:fs/promises'; await writeFile(${JSON.stringify(statePath)}, ${JSON.stringify(staleState)}, 'utf8')`,
    ])
    let released = false
    try {
      const staleRead = await Promise.race([
        writer.exited.then((code) => {
          expect(code).toBe(0)
          return true
        }),
      ...
      await rm(statePath, { force: true })
      await writeFile(path, concurrentConfig, 'utf8')
      await writeFile(statePath, concurrentState, 'utf8')
      await configLock.release()
      released = true
      return await clearPromise
    } finally {
      writer.kill()
      if (!released) await configLock.release()
    }
  }

  test.serial(
    'backs off terminal route.open failures while logging their raw code',
    async () => {
      const logs: LogTestRecord[] = []
      __setLogTestSink((record) => logs.push(record))
      let credentialGets = 0
      ...
      const fixture = await bootRuledClaustrumRow({
        ...
        connector: (calls) =>
          connectorFor(calls, (method, params) => {
            ...
            if (!unavailable)
              return credentialResponse('vault-warm-access', 1, 1)
            credentialGets += 1
            throw new SubcCallError(
              'terminal',
              'route.open failed for module claustrum',
              'missing_identity',
            )
          }),
      })
      unavailable = true
      fixture.setNow(2)
      fixture.plugin.__claustrumCredentialCache.invalidate(fallbackHandle)
      await expect(
        fixture.plugin.__claustrumCredentialCache.get(fallbackHandle),
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          level: 'warn',
          channel: 'claustrum',
          message: 'terminal credential call failed',
          payload: {
            code: 'missing_identity',
            message: 'route.open failed for module claustrum',
          },
        }),
      )
      await fixture.plugin.dispose?.()
  ...
  test.serial(
    'backs off latched vault probes until the next background tick',
    async () => {
      let credentialGets = 0
      let latched = false
      const fallbackHandle = `ckh_${'L'.repeat(43)}`
      const fixture = await bootRuledClaustrumRow({
        route: 'fallback-first',
        initialNow: 0,
        response: new Response('{}', { status: 200 }),
        ...
        connector: (calls) =>
          connectorFor(calls, (method, params) => {
            ...
            if (params.handle === ruledMainHandle)
              return credentialResponse('vault-main-access', 1)
            if (!latched) return credentialResponse('vault-warm-access', 1, 1)
            credentialGets += 1
            return {
              result: { error: { class: 'auth_required', code: 'latched' } },
            }
          }),
      })
      latched = true
      ...
      await expect(
        fixture.plugin.__claustrumCredentialCache.get(fallbackHandle),
      ).rejects.toMatchObject({ code: 'latched' })
      ...
      fixture.setNow(FALLBACK_BACKGROUND_TICK_MS + 1)
      const response = await fixture.result.fetch(MESSAGES_URL, EMPTY_POST)
      expect(response.status).toBe(200)
      await expect(
        fixture.plugin.__claustrumCredentialCache.get(fallbackHandle),
  ...
  test('dark fallback boot does not refresh a plain control beside a vault account', async () => {
    const storage = fallbackWithClaustrum({
      label: 'live-refresh-gate',
    ...
    const saved = await loadAccounts()
    const vault = saved?.accounts.find(
  ...
  test('clearing a sidecar error does not roll back a concurrent routing change', async () => {
    const accountId = 'routing-race'
      ...
      accounts: [
        {
          ...
          lastRefreshError: persistedRefreshError(accountId),
        },
      ],
    })
    await useTempAccountFile(storage)
    const path = process.env.OPENCODE_ANTHROPIC_AUTH_FILE!
    const before = (await loadAccounts(path))?.accounts[0] as
      | OAuthAccount
      | undefined
    expect(before?.lastRefreshError).toBeDefined()
    const concurrent = structuredClone((await loadAccounts(path))!)
    concurrent.routing = { mode: 'fallback-first' }

    const clearResult = await clearAfterConcurrentSnapshot(
      accountId,
      handle,
      concurrent,
      path,
    ...
    const account = saved?.accounts.find(
      (candidate) => candidate.id === accountId,
    ) as OAuthAccount | undefined
    expect(clearResult).toBe(true)
    expect(saved?.routing?.mode).toBe('fallback-first')
    expect(account?.lastRefreshError).toBeUndefined()
  })

  test('clearing a sidecar error does not roll back a concurrent access-token change', async () => {
    ...
    expect(before?.lastRefreshError).toBeDefined()
    const concurrent = structuredClone((await loadAccounts(path))!)
    const account = concurrent.accounts.find(
      (candidate) => candidate.id === accountId,
    )
    if (account?.type !== 'oauth') throw new Error('missing OAuth account')
    account.access = 'new-access-from-refresh'
    ...
    expect(savedAccount?.access).toBe('new-access-from-refresh')
    expect(savedAccount?.lastRefreshError).toBeUndefined()
  })

  test('clears a persisted sidecar refresh error for an enabled Claustrum account', async () => {
    const accountId = 'clear-control'
    ...
    const clearResult = await clearClaustrumRefreshErrorPersistent(
      accountId,
      handle,
      path,
    )
    ...
    expect(clearResult).toBe(true)
    const after = (await loadAccounts(path))?.accounts[0] as
      | OAuthAccount
      | undefined
    expect(after?.lastRefreshError).toBeUndefined()
  })

  test('does not take a locked clear on a clean warm vault request', async () => {
    const accountId = 'clean-warm-request'
    ...
    const fixture = await bootRuledClaustrumRow({
      ...
      fallbacks: [
        {
          label: 'clean-warm-request',
          handle: fallbackHandle,
          access: 'vault-access',
          account: { id: accountId, claustrumHandle: fallbackHandle },
        },
      ],
      connector: (calls) =>
        connectorFor(calls, (method, params) => {
          ...
          if (params.handle === ruledMainHandle)
            return credentialResponse(
              'vault-main-access',
              1,
              Date.now() + FALLBACK_BACKGROUND_TICK_MS + 120_000,
            )
          return credentialResponse(
            'vault-access',
            23,
    ...
    clearCalls.length = 0
    const cleanResponse = await fixture.result.fetch(MESSAGES_URL, EMPTY_POST)
    expect(cleanResponse.status).toBe(200)
    expect(fixture.authorizations).toContain('Bearer vault-access')
    expect(clearCalls).toHaveLength(0)

    const withError = await loadAccounts()
    const account = withError?.accounts.find(
      (candidate) => candidate.id === accountId,
    ) as OAuthAccount | undefined
    if (!withError || !account) throw new Error('expected warm OAuth account')
    account.access = 'stored-fallback-access'
    account.refresh = 'stored-fallback-refresh'
    account.expires = Date.now() + 60_000
    account.claustrumHandle = fallbackHandle
    account.lastRefreshError = {
      ...persistedRefreshError(accountId),
      nextRetryAt: Date.now() - 1,
    }
    await saveAccounts(withError)
    const persistedConfig = JSON.parse(
      await readFile(process.env.OPENCODE_ANTHROPIC_AUTH_FILE!, 'utf8'),
    )
    persistedConfig.accounts.find(
      (candidate: { id: string }) => candidate.id === accountId,
    ).claustrumHandle = fallbackHandle
    await writeFile(
    ...
    expect(
      (
        persisted?.accounts.find((candidate) => candidate.id === accountId) as
          | OAuthAccount
          | undefined
      )?.lastRefreshError,
    ).toBeDefined()

    const erroredResponse = await fixture.result.fetch(MESSAGES_URL, EMPTY_POST)
    expect(erroredResponse.status).toBe(200)
    ...
    expect(clearCalls).toEqual([accountId])
    await fixture.plugin.dispose?.()
  })

  test('leaves a plain OAuth account refresh error untouched', async () => {
    const plainId = 'plain-oauth'
    const controlId = 'enabled-control'
    const plainError = persistedRefreshError(plainId)
      ...
      createFallbackStorage({
        ...
        accounts: [
          ...
          },
          {
            ...
            expires: Date.now() + 60_000,
            claustrumHandle: 'handle-enabled-control',
            lastRefreshError: controlError,
          },
        ],
      }),
    )
    ...
    expect(
      (
        before?.accounts.find((account) => account.id === plainId) as
          | OAuthAccount
          | undefined
      )?.lastRefreshError,
    ).toEqual(plainError)
    expect(
      (
        before?.accounts.find((account) => account.id === controlId) as
          | OAuthAccount
          | undefined
      )?.lastRefreshError,
    ).toEqual(controlError)

    const plainResult = await clearClaustrumRefreshErrorPersistent(
      plainId,
      'not-configured',
      path,
    )
    const controlResult = await clearClaustrumRefreshErrorPersistent(
      controlId,
      'handle-enabled-control',
      path,
    )

    const saved = await loadAccounts(path)
    const plain = saved?.accounts.find((account) => account.id === plainId) as
      | OAuthAccount
      | undefined
    const control = saved?.accounts.find(
      (account) => account.id === controlId,
    ) as OAuthAccount | undefined
    expect(plainResult).toBe(false)
    expect(controlResult).toBe(true)
    expect(plain?.lastRefreshError?.message).toBe(plainError.message)
  ...
  test('clears a persisted sidecar refresh error when the vault later becomes resident', async () => {
    const now = Date.now()
    const plainPermanentError = buildRefreshOperationError({
      error: new ClaudeOAuthRefreshError(400, '{"error":"invalid_grant"}'),
      now,
    ...
    const fallbackHandle = `ckh_${'R'.repeat(43)}`
    const fixture = await bootRuledClaustrumRow({
      route: 'fallback-first',
      fallbacks: [
        {
          ...
          handle: fallbackHandle,
          access: 'vault-recovered-access',
          account: { id: 'vault-recovered' },
        },
      ],
      onFetch: (input) => {
        if (extractUrl(input as string | URL | Request) === TOKEN_URL)
          throw new Error('local refresh must not run')
    ...
    storage.accounts = [
      ...
      },
      {
        ...
        refresh: 'plain-refresh',
        expires: now + 5 * 60 * 60 * 1000,
        lastRefreshError: plainPermanentError,
      },
    ]
    await saveAccounts(storage)
    ...
    config.accounts.find(
      (account: { id: string }) => account.id === 'vault-recovered',
    ).claustrumHandle = fallbackHandle
    await writeFile(
      process.env.OPENCODE_ANTHROPIC_AUTH_FILE!,
    ...
    const saved = await waitForAccountStorage(
      (candidate) =>
        (
    ...
    const plain = saved?.accounts.find(
      (account) => account.id === 'plain-control',
    ) as OAuthAccount
    expect(response.status).toBe(200)
  ...
  test('clears a late local refresh error on a subsequent warm vault request', async () => {
    ...
    const handle = `ckh_${'W'.repeat(43)}`
    const now = Date.now()
    const fixture = await bootRuledClaustrumRow({
      route: 'fallback-first',
      fallbacks: [
        {
          label: 'vault-late-refresh-error',
          handle,
    ...
    account.access = 'expired-vault-access'
    account.refresh = 'expired-vault-refresh'
    account.expires = now - 1
    ...
    expect(response.status).toBe(200)
    expect(fixture.authorizations).toEqual(['Bearer vault-warm-access'])
    expect(
      fixture.calls.filter(
        (call) =>
  ...
  async function loadFallbackWithConnector(
  ...
  test('local mode does not serve or refresh a bound inert fallback', async () => {
    ...
    const connector = async () => {
      connectorCalls += 1
      return connectorFor(calls, () => credentialResponse('vault-access', 1))()
    }
  ...
  test('account status inspects the configured Claustrum connection file', async () => {
    ...
    await useTempAccountFile(storage)
    const connectionFile = join(tempConfigDir!, 'configured-claustrum.json')
    await writeFile(connectionFile, '{ not valid JSON')
    ...
    try {
      ...
      await expectHandledCommandResponse(
        plugin['command.execute.before']({
          command: 'claude-account',
          arguments: '',
          sessionID: 'configured-connection-status',
        }),
      )
      const text = (mockClient.session.promptAsync as any).mock.calls.at(
        -1,
      )?.[0]?.body.parts[0]?.text
      expect(text).toContain('- Claustrum: malformed')
      await plugin.dispose?.()
    } finally {
  ...
  test('account modal payload projects Claustrum status through the real builder', async () => {
    ...
    try {
      ...
      const notification = drainNotifications(0, sessionID).at(-1)
      ...
      expect(
        accounts.find((account) => account.id === 'fallback-1')?.claustrumGate,
      ).toBe('on')
      expect(
        accounts.find((account) => account.id === 'fallback-1')?.vaultServed,
      ).toBe(true)
      expect(
        accounts.find((account) => account.id === 'fallback-1')?.vaultReauth,
      ).toBe(false)
      expect(
        accounts.find((account) => account.id === 'fallback-1')?.custodyState,
      ).toBe('on-vault-served')
      const dialogCustodyState = accounts.find(
        (account) => account.id === 'fallback-1',
      )?.custodyState
      expect(
  ...
  test('treats an empty Claustrum connection setting as unset', async () => {
    ...
    try {
      for (const configuredConnectionFile of ['', ' \t']) {
        process.env.OPENCODE_ANTHROPIC_AUTH_CLAUSTRUM_CONNECTION_FILE =
          configuredConnectionFile
        const calls: CredentialCall[] = []
        let connectionFile: string | undefined
        const connector = async (options: unknown) => {
          connectionFile = (options as { connectionFile?: string })
            .connectionFile
          return connectorFor(calls, (method) => {
            if (method === 'credential.get')
              return credentialResponse('vault-empty-setting', 8)
            return { result: {} }
          })()
        }
  ...
  test('backs off repeated transport Claustrum warm failures per handle', async () => {
    const fallbackHandle = `ckh_${'X'.repeat(43)}`
    let unavailable = false
    const fixture = await bootRuledClaustrumRow({
      route: 'fallback-first',
      initialNow: 0,
      ...
      connector: (calls) =>
        connectorFor(calls, (method, params) => {
          ...
          if (params.handle === ruledMainHandle)
            return credentialResponse('vault-main-access', 1)
          if (!unavailable) return credentialResponse('vault-warm-access', 1, 1)
          throw new Error('vault connection reset')
        }),
    })
    unavailable = true
    fixture.setNow(2)
    ...
    await expect(
      fixture.plugin.__claustrumCredentialCache.get(fallbackHandle),
    ).rejects.toThrow('vault connection reset')
    ...
    const credentialGets = fixture.calls.filter(
      (call) =>
        call.method === 'credential.get' &&
        call.params.handle === fallbackHandle,
    ).length
    ...
    const connector = async () =>
      ({
        call: async (
          ...
          if (method === 'credential.get' && !normalizedOptions.identity) {
            throw new Error('managed call requires a BindIdentity')
          }
          wireCalls.push({ method, options: normalizedOptions })
          return credentialResponse('vault-production-access', 73)
        },
        close: () => {},
      }) as never

    const plugin = await getPlugin(undefined, '/project-root', {
      claustrumConnector: connector,
    })
    const identity = wireCalls.find((call) => call.method === 'credential.get')
      ?.options?.identity as Record<string, unknown> | undefined

    expect(identity).toEqual({
      project_root: '/project-root',
      harness: 'opencode',
      session: `store-${primeStorageFingerprint(
        process.env.OPENCODE_ANTHROPIC_AUTH_FILE!,
      )}`,
    })
    await plugin.dispose?.()
  ...
  test('sticky quota refresh prefers the vault credential over sidecar access', async () => {
      ...
      accounts: [
        {
          id: 'fallback-1',
          type: 'oauth',
          access: 'stale-sidecar-access',
          refresh: 'fallback-refresh',
          expires: Date.now() + 5 * 60 * 60 * 1000,
          claustrumHandle: 'handle-sticky-quota',
          quota: {
            five_hour: {
              usedPercent: 10,
              remainingPercent: 90,
    ...
    const connector = connectorFor(calls, (method, params) => {
      if (method === 'credential.get' && params.handle === ruledMainHandle)
        return credentialResponse('vault-main-access', 91)
      if (method === 'credential.get')
        return credentialResponse('vault-fresh-access', 91)
      return { result: {} }
    })
    const {
      authorizations: messageAuthorizations,
      plugin,
      result,
    } = await loadRuledFallbackWithConnector(
      storage,
    ...
    const quotaManager = plugin.__quotaManager as any
    const originalGetFallback = quotaManager.getFallback.bind(quotaManager)
    const quotaTokens: string[] = []
    quotaManager.getFallback = (accountId: string, account?: unknown) => {
      if (accountId === 'fallback-1') return undefined
      return originalGetFallback(accountId, account)
    }
    quotaManager.refreshFallback = async (
      _accountId: string,
      accessToken: string,
    ) => {
      quotaTokens.push(accessToken)
      return {
        five_hour: {
          usedPercent: 10,
          remainingPercent: 90,
    ...
    const response = await result.fetch(MESSAGES_URL, {
      method: 'POST',
      headers: { 'x-session-affinity': 'sticky-quota-session' },
      body: JSON.stringify({
        model: 'claude-opus-5',
        max_tokens: 1,
        messages: [{ role: 'user', content: 'hello' }],
    ...
    expect(response.status).toBe(200)
    expect(messageAuthorizations).toContain('Bearer vault-fresh-access')
    expect(quotaTokens).toEqual(['vault-fresh-access'])
    await plugin.dispose?.()
  })
  ...
  test('skips a Claustrum account when the vault credential is expired', async () => {
    ...
    const fixture = await bootRuledClaustrumRow({
      ...
      fallbacks: [
        {
          label: 'expired',
          handle: `ckh_${'X'.repeat(43)}`,
          access: 'vault-expired-access',
        },
        {
          label: 'backup',
          handle: `ckh_${'Y'.repeat(43)}`,
          access: 'vault-backup-access',
        },
      ],
      connector: (calls) =>
        connectorFor(calls, (method, params) => {
          if (method !== 'credential.get') return { result: {} }
          if (params.handle === ruledMainHandle)
            return credentialResponse('vault-main-access', 31, 10_000)
          if (params.handle === `ckh_${'X'.repeat(43)}`)
            return credentialResponse('vault-expired-access', 31, 2_000)
          return credentialResponse('vault-backup-access', 31, 10_000)
        }),
      onFetch: (input, init) => {
        if (extractUrl(input as string | URL | Request) === TOKEN_URL) {
          tokenRefreshes.push(
    ...
    expect(response.status).toBe(200)
    expect(fixture.authorizations).toEqual(['Bearer vault-backup-access'])
    expect(fixture.authorizations).not.toContain('Bearer vault-expired-access')
  ...
  test('bounds startup warmup and continues with a cold account', async () => {
    ...
    const fixture = await bootRuledClaustrumRow({
      ...
      fallbacks: [
        {
          label: 'wedged',
          handle: `ckh_${'W'.repeat(43)}`,
          access: 'vault-warm-access',
        },
      ],
      connector: (calls) =>
        connectorFor(calls, async (method, params) => {
          if (method !== 'credential.get') return { result: {} }
          if (params.handle === ruledMainHandle)
            return credentialResponse('vault-main-access', 1, 10_000)
          if (cold) await releaseCold.promise
          return credentialResponse('vault-warm-access', 1, 2_000)
        }),
    })
    try {
      cold = true
      ...
      const response = await withDeadlockGuard<Response>(
        fixture.result.fetch(MESSAGES_URL, EMPTY_POST),
        150,
        'cold vault request exceeded 150ms',
      )
  ...
  test('detaches a stale-marked startup refresh and uses its result when it lands', async () => {
    ...
    const fixture = await bootRuledClaustrumRow({
      ...
      connector: (calls) =>
        connectorFor(calls, async (method, params) => {
          if (method !== 'credential.get') return { result: {} }
          if (params.handle === ruledMainHandle)
            return credentialResponse('vault-main-access', 52, 10_000)
          if (cold) {
            await warmGate
            return credentialResponse('vault-after-stale-refresh', 52, 10_000)
          }
          return credentialResponse('vault-warm-access', 52, 2_000)
        }),
    })
    cold = true
    fixture.setNow(3_000)
    ...
    releaseWarm()
    await Bun.sleep(150)
    const refreshedResult = await fixture.plugin.auth.loader(
      () => Promise.resolve(custodyTombstoneOAuth('anthropic') as never),
      { models: {} },
    )
    const response = await refreshedResult.fetch(MESSAGES_URL, EMPTY_POST)

    expect(response.status).toBe(200)
    expect(fixture.authorizations).toContain('Bearer vault-after-stale-refresh')
    await fixture.plugin.dispose?.()
  })
    ...
    try {
      const response = await fixture.result.fetch(MESSAGES_URL, {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: 'user', content: 'relay auth failure' }],
        }),
      })
      expect(response.status).toBe(200)
      expect(await response.text()).toContain('relay_upstream_error')
    } finally {
      restoreWebSocket()
    }
    ...
    try {
      const response = await fixture.result.fetch(MESSAGES_URL, {
        ...
        body: JSON.stringify({
          stream: true,
          model: 'claude-opus-5',
          messages: [{ role: 'user', content: 'sticky relay auth failure' }],
        }),
      })
      expect(response.status).toBe(200)
      expect(await response.text()).toContain('relay_upstream_error')
  ...
  test('does not report relay 401s for non-vault routes', async () => {
      ...
      accounts: [
        {
          id: 'plain-fallback',
          type: 'oauth',
          access: 'plain-fallback-access',
          refresh: 'plain-fallback-refresh',
          expires: Date.now() + 5 * 60 * 60 * 1000,
        },
      ],
    })
    ...
    try {
      const response = await result.fetch(MESSAGES_URL, {
        method: 'POST',
        headers: { 'x-session-affinity': 'relay-plain-401' },
        body: JSON.stringify({
          stream: true,
          model: 'claude-sonnet-4-5',
          messages: [{ role: 'user', content: 'plain relay auth failure' }],
        }),
      })
      expect(response.status).toBe(200)
      expect(await response.text()).toContain('relay_upstream_error')
  ...
  test('cold custody never refreshes a sidecar after a 401', async () => {
    ...
    const fallbackHandle = `ckh_${'O'.repeat(43)}`
    let unavailable = false
    let tokenCalls = 0
    const fixture = await bootRuledClaustrumRow({
      route: { sticky: 'fallback-1' },
      initialNow: 0,
      quota: {
        enabled: true,
        checkIntervalMinutes: 5,
    ...
    const response = await fixture.result.fetch(MESSAGES_URL, {
      method: 'POST',
      headers: { 'x-session-affinity': 'vault-outage-recovery' },
      body: JSON.stringify({
        model: 'claude-opus-5',
        max_tokens: 1,
        messages: [{ role: 'user', content: 'hello' }],
    ...
    async () => {
      ...
      try {
        const fixture = await bootRuledClaustrumRow({
          ...
          onFetch: async (input, init) => {
            ...
            if (url.includes('/v1/messages')) {
              ...
              if (authorization === 'Bearer vault-old-access') {
                cache.invalidate(fallbackHandle, 41)
                await cache.get(fallbackHandle)
                return new Response('{}', { status: 401 })
              }
        ...
        const record = logs.find(
          (candidate) => candidate.message === 'vault-served 401 recovery',
        )
        expect(record?.payload?.retryAttempted).toBe(true)
        expect(record?.payload?.retryOutcome).toBe('retry-succeeded')
        expect(record?.payload?.reportOutcome).toBe('not-attempted')
        expect(record?.payload?.retryServedRecordVersion).toBeGreaterThan(
          record?.payload?.servedRecordVersion as number,
        )
      } finally {
        await plugin?.dispose?.()
        __setLogTestSink(null)
      ...
      try {
        const fixture = await bootRuledClaustrumRow({
          ...
          fallbacks: [
            {
              label: 'unchanged-401',
              handle: `ckh_${'N'.repeat(43)}`,
              access: 'vault-unchanged-access',
            },
          ],
          connector: (calls) =>
            connectorFor(calls, (method, params) => {
              ...
              if (String(params.handle) === ruledMainHandle)
                return credentialResponse('vault-main-access', 1)
              return credentialResponse('vault-unchanged-access', 52)
            }),
          response: new Response('{}', { status: 401 }),
        })
        plugin = fixture.plugin
        ...
        expect(record?.payload?.retryAttempted).toBe(false)
        expect(record?.payload?.vaultGetAttempted).toBe(true)
        expect(record?.payload?.retryOutcome).toBe('unchanged')
        expect(record?.payload?.reportOutcome).toBe('reported')
        expect(record?.payload?.servedRecordVersion).toBe(
  ...
  test.serial('logs which branch suppressed a raced 401 report', async () => {
    const logs: LogTestRecord[] = []
    __setLogTestSink((record) => logs.push(record))
    let cache: {
      invalidate: (handle: string, recordVersion?: number) => void
    ...
    try {
      const fixture = await bootRuledClaustrumRow({
        ...
        connector: (calls) =>
          connectorFor(calls, (method, params) => {
            ...
            fallbackGets += 1
            if (fallbackGets === 1)
              return credentialResponse('vault-old-access', 41)
            if (fallbackGets === 2)
              return credentialResponse('vault-new-access', 42)
            throw new Error('retry credential unavailable')
          }),
        onFetch: async (input, init) => {
          const url = extractUrl(input as string | URL | Request)
          if (url.includes('/v1/messages')) {
      ...
      const record = logs.find(
        (candidate) =>
          candidate.message === 'vault-served 401 recovery' &&
          String(candidate.payload?.reportOutcome).startsWith('suppressed-'),
      )
      expect(String(record?.payload?.reportOutcome)).toMatch(/^suppressed-/)
      expect(record?.payload?.reportSuppressed).toBe(true)
      expect(record?.payload?.reportSuppressedBy).toBe(
        record?.payload?.reportOutcome,
      )
    } finally {
      await plugin?.dispose?.()
      __setLogTestSink(null)
  ...
  test.serial(
    'logs that no vault get was attempted during active retry backoff',
    async () => {
      const logs: LogTestRecord[] = []
      __setLogTestSink((record) => logs.push(record))
      let plugin: { dispose?: () => Promise<void> } | undefined
      ...
      try {
            ...
            connectorFor(calls, (method, params) => {
              ...
              if (failStartup)
                throw new ClaustrumCredentialError(
                  'startup credential unavailable',
                  'test_transient',
                  'transient',
                  'retry',
                )
              return credentialResponse('vault-backoff-access', 61)
            }),
          onFetch: (input, init) => {
            const authorization = new Headers(init?.headers).get(
              'authorization',
            )
            if (authorization === 'Bearer vault-backoff-access')
              return new Response('{}', { status: 401 })
            return new Response('{}', { status: 200 })
          },
        })
        plugin = fixture.plugin
        const cache = fixture.plugin.__claustrumCredentialCache as any
        failStartup = false
        const servedCredential = await cache.get(fallbackHandle)
        ...
        const record = logs.find(
          (candidate) =>
            candidate.message === 'vault-served 401 recovery' &&
            candidate.payload?.retryOutcome === 'backoff-active',
        )
        if (!record) throw new Error(JSON.stringify(logs))
        expect(record?.payload?.vaultGetAttempted).toBe(false)
      } finally {
        await plugin?.dispose?.()
        __setLogTestSink(null)
      }
...
describe('AnthropicAuthPlugin', () => {
  ...
  test.serial(
    ...
    async () => {
      ...
      const servedRecordVersion = 9
      const client = createMockClient()
      const fixture = await bootSharedRuledClaustrumRow({
        route: 'fallback-first',
        fallbacks: [{ label, handle, access: 'vault-a-access' }],
        connector: (calls) =>
          connectorFor(calls, (method, params) => {
            if (method !== 'credential.get') return { result: {} }
            return credentialResponse(
              params.handle === handle ? 'vault-a-access' : 'vault-main-access',
              params.handle === handle ? servedRecordVersion : 1,
            )
          }),
        onFetch: (input, init) => {
          const url = extractUrl(input as string | URL | Request)
        ...
        getPlugin: async (accountStoragePath, runtimeOverrides) => {
          process.env.OPENCODE_ANTHROPIC_AUTH_FILE = accountStoragePath
          return getPlugin(client, undefined, {
            ...runtimeOverrides,
            authorize,
          } as never)
        },
      ...
      try {
        const response = await fixture.result.fetch(MESSAGES_URL, EMPTY_POST)
        await response.text()
        expect(fixture.authorizations).toContain('Bearer vault-a-access')
        expect(
          fixture.calls.some(
            (call) =>
              call.method === 'credential.get' && call.params.handle === handle,
  ...
  test('fresh install under local with a tombstoned slot', async () => {
    ...
    const dark = await plugin.auth.loader(
      () => Promise.resolve(custodyTombstoneOAuth('anthropic') as never),
      { models: {} },
    )
    await expect(dark.fetch(MESSAGES_URL, EMPTY_POST)).rejects.toMatchObject({
      code: 'custody_state_mismatch',
      verdict: 'REMAIN_DARK_PENDING_LOGIN',
    })
  ...
  test('background refresh keeps vault-enabled fallback freshness in Claustrum', async () => {
    const now = 1_000_000
    let currentNow = now
    ...
    const plugin = await getPlugin(undefined, undefined, {
      ...
      claustrumNow: () => currentNow,
      setInterval,
    })
    await plugin.__fallbackRefreshReady
    currentNow = vaultExpiry + 1
    await tick?.()

    expect(localRefresh).not.toHaveBeenCalled()
    const credentialGets = calls.filter(
      (call) => call.method === 'credential.get',
    )
    expect(credentialGets.length).toBeGreaterThanOrEqual(2)
    expect(credentialGets.at(-1)?.args).toEqual(
      expect.objectContaining({
        handle,
        min_ttl_ms: 240 * 60_000 + 30 * 60_000,
      }),
    )
    installDefaultFetchMock()
  })
  ...
  test('vault reauth leaves the account absent and projects the reauth state', async () => {
    ...
    const sidebar = await waitForSidebarState((state) => {
      ...
      return account?.vaultReauth === true
    })
  ...
  test('transient vault failure preserves an unexpired sidecar without local refresh', async () => {
    const accountId = 'vault-transient-healthy'
    const handle = 'vault-transient-healthy-handle'
    const localRefresh = mock(() =>
      Promise.resolve(new Response('{}', { status: 200 })),
    ) as unknown as typeof fetch
    const client = {
      call: mock(async () => ({
        result: { error: { class: 'transient', code: 'unreachable' } },
      })),
      close: mock(() => {}),
    }
    let tick: (() => Promise<unknown>) | undefined
  ...
  test('transient vault failure leaves an expired sidecar cold without local refresh', async () => {
    ...
    const client = {
      call: mock(async () => ({
        result: { error: { class: 'transient', code: 'timeout' } },
      })),
      close: mock(() => {}),
    }
    let tick: (() => Promise<unknown>) | undefined
  ...
  test('connects when vault custody is enabled after boot', async () => {
    const accountId = 'mid-session-vault'
    const handle = 'mid-session-vault-handle'
    let connectAttempts = 0
    let credentialGets = 0
    let tick!: () => Promise<unknown>
    const setInterval = mock((callback: () => Promise<unknown>) => {
      if (!tick) tick = callback
      return { unref() {} } as unknown as ReturnType<typeof setInterval>
    }) as unknown as typeof globalThis.setInterval
    const connector = async () => {
      connectAttempts += 1
        ...
        call: mock(async () => {
          ...
          return {
            result: {
              payload: Array.from(
                new TextEncoder().encode(
                  JSON.stringify({ access_token: 'mid-session-vault-access' }),
                ),
              ),
              expires_at_ms: Date.now() + 60 * 60_000,
              record_version: credentialGets,
            },
          }
        }),
        close: mock(() => {}),
  ...
  test('backs off failed mid-session vault connections', async () => {
    const accountId = 'mid-session-vault-failure'
    let now = 0
    let connectAttempts = 0
    let tick!: () => Promise<unknown>
    const setInterval = mock((callback: () => Promise<unknown>) => {
        ...
        accounts: [
          {
            ...
            refresh: 'sidecar-refresh',
            expires: Date.now() + 5 * 60 * 60_000,
            claustrumHandle: 'mid-session-vault-failure-handle',
          },
        ],
      }),
    )
  ...
  test('custody latch merge prefers the newer error by checkedAt', async () => {
    const accountId = 'vault-fence-recency'
    const handle = 'vault-fence-recency-handle'
    const now = Date.now()
  ...
  test('admits a sticky route with a vault credential and no sidecar access token', async () => {
    ...
    const vaultCredentialResponse = () => ({
      result: {
        payload: Array.from(
          new TextEncoder().encode(
            JSON.stringify({ access_token: 'vault-only-sticky-access' }),
          ),
        ),
    ...
    const request = {
      method: 'POST',
      headers: { 'x-session-affinity': 'vault-only-sticky-session' },
      body: JSON.stringify({
        model: 'claude-opus-5',
        max_tokens: 1,
        messages: [{ role: 'user', content: 'hello' }],
  ...
  test('does not retry a rejected sticky vault credential', async () => {
    ...
    const rejectedVaultCredentialResponse = () => ({
      result: {
        payload: Array.from(
          new TextEncoder().encode(
            JSON.stringify({ access_token: 'rejected-vault-access' }),
          ),
        ),
    ...
    let releaseInitialCredential!: () => void
    const initialCredential = new Promise<unknown>((resolve) => {
...
describe('killswitch fetch gate', () => {
  ...
  async function runVaultKillswitchQuotaRefresh(vaultResident: boolean) {
    ...
    try {
      ...
      let fallbackResident = vaultResident
      const quota = {
      ...
      const custodyTicks: Array<() => unknown> = []
        ...
        connector: (calls) =>
          connectorFor(calls, (method, params) =>
            method !== 'credential.get'
              ? { result: {} }
              : params.handle === handle && !fallbackResident
                ? (() => {
                    throw new Error('vault fallback is cold')
                  })()
                : credentialResponse(
                    params.handle === handle
                      ? vaultAccess
      ...
      const response = await fixture.result.fetch(MESSAGES_URL, EMPTY_POST)
      await response.text()
      const coldFallbackUsageCalls = usageAuthorizations.filter(
        (authorization) => authorization === `Bearer ${vaultAccess}`,
      ).length
      const coldSidecarUsageCalls = usageAuthorizations.filter(
      ...
      if (!vaultResident) {
        ...
        await Promise.all(custodyTicks.map((handler) => handler()))
        residentAfterTick = Boolean(
          plugin.__claustrumCredentialCache.peek(handle),
        )
        usageAuthorizations.length = 0
        await plugin.__quotaManager.refreshFallback(accountId, vaultAccess)
        recoveredFallbackUsageCalls = usageAuthorizations.filter(
          (authorization) => authorization === `Bearer ${vaultAccess}`,
        ).length
      }
      await plugin.dispose?.()
  ...
  test('killswitch quota refresh polls with a resident vault credential instead of an expired sidecar', async () => {
    ...
    expect(result.residentAfterTick).toBe(true)
  })

  test('killswitch quota refresh skips a cold vault fallback without local refresh', async () => {
    const result = await runVaultKillswitchQuotaRefresh(false)

    expect(result.residentBeforeRequest).toBe(false)
    expect(result.sidecarUsageCalls).toBe(0)
    expect(result.fallbackUsageCalls).toBe(0)
    expect(result.scheduledWarmCount).toBe(0)
    expect(result.recoveredFallbackUsageCalls).toBe(1)
  })
